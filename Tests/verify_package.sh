...
case "$phase" in
  ...
  metadata)
    ...
    test "$(dpkg-deb -f "$deb" Version)" = "0.1.0~beta57"
...
assert app_info["CFBundleVersion"] == "57"
...
PY
  ...
  markers)
    ...
    grep -Fq 'Backup Not Portable' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq -- '--download-only' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq 'APT::Get::AllowUnauthenticated=false' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq 'APT::Get::Allow-Downgrades=false' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq 'APT::Get::Allow-Change-Held-Packages=false' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq 'Acquire::AllowInsecureRepositories=false' "$RUNNER_TEMP/aaz-app.strings"
