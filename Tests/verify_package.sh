...
case "$phase" in
  ...
  metadata)
    ...
    test "$(dpkg-deb -f "$deb" Version)" = "0.1.0~beta31"
...
assert app_info["CFBundleVersion"] == "31"
...
PY
  ...
  markers)
    ...
    grep -Fq 'R31-READINESS' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq 'R31-BLOCKED' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq 'R31-OK' "$RUNNER_TEMP/aaz-app.strings"
