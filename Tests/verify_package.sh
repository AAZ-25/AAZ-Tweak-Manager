...
case "$phase" in
  ...
  metadata)
    ...
    test "$(dpkg-deb -f "$deb" Version)" = "0.1.0~beta35"
...
assert app_info["CFBundleVersion"] == "35"
...
PY
  ...
  markers)
    ...
    grep -Fq 'R35-READINESS' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq 'R35-BLOCKED' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq 'R35-PREFLIGHT' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq 'R35-OK' "$RUNNER_TEMP/aaz-app.strings"
    ...
    grep -Fq 'R35-DPKG-PREFLIGHT' "$RUNNER_TEMP/aaz-app.strings"
    grep -Fq 'R35-PARTIAL' "$RUNNER_TEMP/aaz-app.strings"
