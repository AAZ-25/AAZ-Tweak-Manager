...
- (NSDictionary *)restoreReadinessForBackupURL:(NSURL *)backupURL password:(NSString *)password installedPackages:(NSArray<ATMPackageRecord *> *)installed error:(NSError **)error {
    ATMSetRestoreDiagnosticState(@"R34-READINESS", -1);
    NSDictionary *manifest = [self prepareRestoreSessionForBackupURL:backupURL password:password error:error];
    if (!manifest) { ATMSetRestoreDiagnosticState(@"R34-BLOCKED", error && *error ? (*error).code : -1); return nil; }
    ...
    if (!plan) { ATMSetRestoreDiagnosticState(@"R34-BLOCKED", error && *error ? (*error).code : -1); [self clearRestoreSession]; return nil; }
    NSMutableDictionary *sessionPlan = [plan mutableCopy]; sessionPlan[@"restoreSessionID"] = self.restoreSessionID;
    ATMSetRestoreDiagnosticState([plan[@"safeToExecute"] boolValue] || ([plan[@"simulationPassed"] boolValue] && [plan[@"blocked"] unsignedIntegerValue] == 0) ? @"R34-READY" : @"R34-BLOCKED", [plan[@"aptExitCode"] integerValue]);
    return sessionPlan;
}
- (NSDictionary *)executeRestoreForManifest:(NSDictionary *)manifest expectedPlan:(NSDictionary *)expectedPlan error:(NSError **)error {
    ATMSetRestoreDiagnosticState(@"R34-STARTED", -1);
    ...
    if (!sessionValid) { if (error) *error = ATMBackupError(78, @"The verified Restore session expired. Run the readiness check again."); ATMSetRestoreDiagnosticState(@"R34-PRECHECK", 78); [self clearRestoreSession]; return nil; }
