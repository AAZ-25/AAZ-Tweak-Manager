...
- Keeps each import diagnostic tied to the current app build and a random privacy-safe attempt ID, so a stored result from an older build is reported as not run. Import and Restore failures offer one count-only privacy-safe report without filenames, package identities, paths, repository URLs, passwords, or raw tool output.
...
- Restores only missing packages and required upgrades, then restores sanitized public source files after a separate final confirmation and an immediate plan recheck. A source-only Restore is allowed when verified source files are genuinely pending; a true package-and-source no-op remains disabled.
...
Before any package mutation, Restore verifies the sanitized source payload hashes, destination type, existing-file hashes, required tools, and destination writability, and includes the source state in the approved plan snapshot. It repeats that source check immediately before execution and stops on drift. After package verification succeeds, genuinely pending sanitized public source files may be restored under deterministic app-owned names without overwriting existing source files. Sources that contained private credentials are skipped, and newly created source files are rolled back together if any source write fails. Temporary payloads are app-owned, size-limited, never shown in diagnostics, and removed after execution. A count-only completion record is retained locally without exposing package identities.
